Feature: CSX Regression
  ...
  Scenario Outline: Verify place order , get order  and cancel order api
    ...
    Examples: 
      ...
      | limit | BUY  | ETH/INR    | QUOTE        |    10000 |        102 | user1    |

  @UpdateOrder @Regression @ETH
  Scenario: Verify Update Order api
    Given Placing an order for broker "B2" of type "limit" side "BUY" instrument "ETH/INR" quantityType "QUOTE" quantity "10000" limitPrice "102" username "user1"
    When Providing request PathUrl "PlaceOrder" with method "POST"
    Then I verify the  "200" in step
    And Verify key "message" of response "Order placed successfully"
    And Verify the "data.orderID" of response "GetOrder"
    And Verify key "data.status" of response "OPEN"
    Given Update an order for broker "B2" of  instrument "ETH/INR" quantityType "QUOTE" quantity "103.5" limitPrice "103" username "user1" orderType "LIMIT"
  ...
  Scenario: Verify funds for diffrent broker in fulfilled case
    ...
    Given Placing an order for broker "B2" of type "limit" side "SELL" instrument "ETH/INR" quantityType "QUOTE" quantity "10000" limitPrice "3500000" username "user1"
    When Providing request PathUrl "PlaceOrder" with method "POST"
    Then I verify the  "200" in step
    And Verify key "message" of response "Order placed successfully"
    And Verify the "data.orderID" of response "GetOrder"
  ...
  Scenario: Verify funds for diffrent broker in partially fulfilled case
    ...
    Given take balance of "INR" with api "Getbalance" with name "s2b2inr"
    Given take balance of "ETH" with api "Getbalance" with name "s2b2btc"
    Given Placing an order for broker "B2" of type "limit" side "SELL" instrument "ETH/INR" quantityType "QUOTE" quantity "5000" limitPrice "3500000" username "user1"
    When Providing request PathUrl "PlaceOrder" with method "POST"
    Then I verify the  "200" in step
    And Verify key "message" of response "Order placed successfully"
    And Verify the "data.orderID" of response "GetOrder"
  ...
  Scenario: Verify funds for diffrent broker in price improvement case
    ...
    Given take balance of "INR" with api "Getbalance" with name "s2b2inr"
    Given take balance of "ETH" with api "Getbalance" with name "s2b2btc"
    Given Placing an order for broker "B2" of type "limit" side "SELL" instrument "ETH/INR" quantityType "QUOTE" quantity "5000" limitPrice "1750000" username "user1"
    When Providing request PathUrl "PlaceOrder" with method "POST"
    Then I verify the  "200" in step
    And Verify key "message" of response "Order placed successfully"
    And Verify the "data.orderID" of response "GetOrder"
  ...
  @GetBalanceforSameBroker4 @Regression @ETH
  ...
  @GetBalanceforSameBroker5 @Regression @ETH
  ...
  @BUYSELLBUYorders @Regression @ETH
